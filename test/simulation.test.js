...
test('Einheit-Faktor: Leistung W (Standard) und kW', () => {
    assert.strictEqual(unitFactor('W'), 1);
    assert.strictEqual(unitFactor('kW'), 1000); // kW -> W
});

test('Einheit-Faktor: Zählerstand kWh (Standard) und Wh', () => {
    assert.strictEqual(unitFactor('kWh'), 1);
    assert.strictEqual(unitFactor('Wh'), 0.001); // Wh -> kWh
});
